...
def main() -> None:
    ...
    # load metadata written by generate_graphs.py to get the true factor count and step size
    meta_path = script_dir / "metadata.json"
    if not meta_path.exists():
        print("[run_experiment] metadata.json not found — run generate_graphs.py first")
        return
    ...
    for pkl_path in pkl_files:
        ...
        for engine_name, cfg in ENGINE_CONFIGS.items():
            ...
            out_path = results_dir / f"results_graph_{variant_idx:02d}_{engine_name}.csv"
